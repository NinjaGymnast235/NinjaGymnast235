/*
    ...
    https://github.com/Artenes/javascript-snake-game/blob/master/snake.js
...
class Snake {
  ...
  /*
        ...
        If xSpeed is not equal to 0, that means the snake
  ...
  update() {
    ...
    /**
        ...
        */
        ...
        if (this.x > this.game.width - 1) this.x = 0;
        if (this.x < 0) {
          this.x = this.game.width - 1;
        }
        if (this.y > this.game.height - 1) this.y = 0;
        if (this.y < 0) {
    ...
    for (var index = this.parts.length - 1; index >= 0; index--) {
      ...
      this.game.grid.fillTile(part.x, part.y, "#1b4f8a"); //snake color
    }
...
class Food {
  ...
  //Render the food in the grid. (food color)
  update() {
    this.game.grid.fillTile(this.x, this.y, "#a10303");
...
class RenderGrid {
  ...
  update() {
    ...
    for (var x = 0; x < this.game.width; x++) {
      ...
      for (var y = 0; y < this.game.height; y++) {
        var tile = this.grid[x][y];
        var newBackgroundColor = tile.isFilled ? tile.color : "#1daf77"; //canvas/grid color
...
class Game {
  constructor(
    ...
    this.interval = setInterval(function () {
      _this.update();
    }, 3500 / this.fps);  //Snake Speed (as delay in milliseconds)
  }
